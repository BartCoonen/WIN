...
function [ C1_initial,C2_initial,C1_man,C2_man] = Channel_initial( dt )
...
%for transmitter 1: 
    ...
    %get the channel for initial nulling
    fix_loss_1_real = sqrt(10.^(fix_loss_1./10));
    fix_loss_2_real = sqrt(10.^(fix_loss_2./10));
    fix_phase_delay_1 = exp(-2i*pi*fc.*fix_delay_1);
    fix_phase_delay_2 = exp(-2i*pi*fc.*fix_delay_2);
    C1_initial = zeros();%channel response without the man
    C2_initial = zeros();%channel response without the man 
    for i =1:1:4
        C1_initial(1,ceil(fix_delay_1(i,1)/dt))= fix_loss_1_real(i,1)*fix_phase_delay_1(i,1);
        C2_initial(1,ceil(fix_delay_2(i,1)/dt))= fix_loss_2_real(i,1)*fix_phase_delay_2(i,1);
    end
    channel_p = 0.1;
